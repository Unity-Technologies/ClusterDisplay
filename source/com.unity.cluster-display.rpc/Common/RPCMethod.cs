...
namespace Unity.ClusterDisplay.RPC
{
    ...
    public class ClusterRPC : Attribute 
    {
        ...
        private int ValidateRPCId (int id)
        {
            ...
            #if UNITY_EDITOR
            ClusterDebug.Assert((ulong)rpcId < Constants.MaxRPCID, $"You should not be overriding the RPC ID with: ({rpcId}) when it's larger then the max ID ({Constants.MaxRPCID}).");
            #endif
