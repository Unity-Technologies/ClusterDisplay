    ...
    public static partial class ReflectionUtils
    {
            ...
            ref ushort structureDepth)
        {
            ...
            else if (type.IsArray)
            {
                ...
                {
                    LogWriter.LogError($"Generic type argument: \"{type.GenericTypeArguments[0].Name}\" cannot be used as an RPC parameter.");
                    goto dynamicallySizedMemberTypeFailure;
                }
            }
            ...
            else if (type.IsGenericType)
            {
                if (type.Namespace == NativeArrayType.Namespace)
                {
                    ...
                    {
                        LogWriter.LogError($"Only the type: \"{NativeArrayType.Name}\" is supported as a RPC method parameter from: \"{NativeArrayType.Namespace}\" namespace.");
                        goto dynamicallySizedMemberTypeFailure;
                    }
                }
            ...
            {
                LogWriter.LogError($"Parameter: \"{parameterInfo.Name}\" is a reference type. Only primitive, strings, arrays, native arrays and structs are RPC compatible.");
                goto dynamicallySizedMemberTypeFailure;
            }
            ...
            LogWriter.LogError($"Method: \"{methodInfo.Name}\" declared in type: \"{methodInfo.DeclaringType.Name}\" cannot be used as an RPC, the parameter: \"{parameterInfo.Name}\" is type: \"{parameterInfo.ParameterType.Name}\" which contains dynamically allocated type: \"{type.Name}\" somehwere in it's member hierarchy.");
            return false;
        }
    }
}
