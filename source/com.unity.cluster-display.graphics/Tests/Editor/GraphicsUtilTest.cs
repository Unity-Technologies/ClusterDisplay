...
namespace Unity.ClusterDisplay.Graphics.EditorTests
{
    ...
    class GraphicsUtilTest
    {
        ...
        {
            return new Vector4(r.width, r.height, r.x, r.y);
        }
        ...
        {
            return Viewport.TileIndexToSubSection(gridSize, tileIndex);
        }
        ...
        {
            return Viewport.ApplyOverscan(normalizedViewportSubsection, overscanInPixels, viewportWidth, viewportHeight);
        }
        ...
        [TestCase(29)]
        [TestCase(67)]
        public void ScreenCoordOverrideParams(int seed)
        {
            ...
            var postEffectsParams = new PostEffectsParams(globalScreenSize);

            var screenCoordScaleBias = PostEffectsParams.GetScreenCoordScaleBias(overscannedViewportSubsection);
            Assert.IsTrue(screenCoordScaleBias == RectToVector4(overscannedViewportSubsection));

            var screenSizeOverride = postEffectsParams.GetScreenSizeOverride();
            Assert.AreEqual(screenSizeOverride.x, globalScreenSize.x);
            Assert.AreEqual(screenSizeOverride.y, globalScreenSize.y);
            Assert.AreEqual(screenSizeOverride.z, 1f / globalScreenSize.x);
            Assert.AreEqual(screenSizeOverride.w, 1f / globalScreenSize.y);
        }
        ...
        static void SliceProjection(Matrix4x4 projection, Vector2Int gridSize, List<Plane> output)
        {
            ...
            {
                var tile = Viewport.TileIndexToSubSection(gridSize, i);
                var asymmetricProjection = projection.GetFrustumSlice(tile);
