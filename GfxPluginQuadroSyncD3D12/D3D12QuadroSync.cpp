...
namespace GfxQuadroSync
{
	...
	bool PluginCSwapGroupClient::Initialize(ID3D12Device* const pDevice,
	...
	void PluginCSwapGroupClient::Dispose(ID3D12Device* const pDevice,
		IDXGISwapChain* const pSwapChain)
	{
		NvAPI_Status status;
		if (m_GroupId > 0)
	...
	NvU32 PluginCSwapGroupClient::QueryFrameCount(ID3D12Device* const pDevice)
	{
		NvU32 count = 0;
	...
	// Same
	void PluginCSwapGroupClient::ResetFrameCount(ID3D12Device* const pDevice)
	{
		if (m_GSyncMaster)
		{
			auto status = NVAPI_OK;
	...
	bool PluginCSwapGroupClient::Render(ID3D12Device* const pDevice,
		IDXGISwapChain* const pSwapChain,
		const int pVsync,
	...
	void PluginCSwapGroupClient::EnableSystem(ID3D12Device* const pDevice,
		IDXGISwapChain* const pSwapChain,
		const bool value)
	{
		m_IsActive = value;
	...
	void PluginCSwapGroupClient::EnableSwapGroup(ID3D12Device* const pDevice,
		IDXGISwapChain* const pSwapChain,
		const bool value)
	{
#ifdef DEBUG_LOG_QUADRO_SYNC
	...
	void PluginCSwapGroupClient::EnableSwapBarrier(ID3D12Device* const pDevice,
		const bool value)
	{
		if (m_GroupId == 1)
		{
