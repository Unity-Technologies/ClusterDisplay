...
def main():
    ...
    if args.startCluster is not None:
        ...
        subprocess.Popen('c:\\PSTools\\PsExec.exe \\\\' + config['servers'][0]['name'] + ' -i -s ' + path + ' -useClusterRendering -masterNode 0 3 224.0.1.0:25689,25690 30')
        subprocess.Popen('c:\\PSTools\\PsExec.exe \\\\' + config['servers'][1]['name'] + ' -i -s ' + path + ' -useClusterRendering -node 1 224.0.1.0:25690,25689 30')
        subprocess.Popen('c:\\PSTools\\PsExec.exe \\\\' + config['servers'][2]['name'] + ' -i -s ' + path + ' -useClusterRendering -node 2 224.0.1.0:25690,25689 30')
        subprocess.Popen('c:\\PSTools\\PsExec.exe \\\\' + config['servers'][3]['name'] + ' -i -s ' + path + ' -useClusterRendering -node 3 224.0.1.0:25690,25689 30')
